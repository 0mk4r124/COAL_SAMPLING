...
DB_POLL_SEC     = 10     # how often to re-check DB when RFID not yet registered
DB_WAIT_TIMEOUT = 300    # seconds before giving up waiting for RFID in DB
TOTAL_CYCLES = 3
SAVE_PATH = "C:/Users/COAL_SAMPLING_1/PRODUCTION_CODE/COAL_SAMPLING/TEMP_IMG/"

SAMPLE_POSITIONS = [
    {"x": 100, "y": 150},   # front-left
    {"x": 300, "y": 150},   # centre
    {"x": 500, "y": 150},   # rear-right
]

# Shuffle positions so sampling is randomised each run
...
class State(Enum):
    ...
    CYCLE_POSITION = auto()   # sent set_position, waiting for ack
    CYCLE_CAPTURE = auto()   # cam13 + cycle running, waiting for discharge
    CYCLE_DONE = auto()   # discharge received, decide next/green
    GREEN_SIGNAL = auto()
    COMPLETE = auto()
    ERROR = auto()
...
def db_complete_log(uid: str) -> bool:
    """Mark a VEHICLE_LOGS row as COMPLETED."""
    db = None
    try:
        db  = _db_connect()
        cur = db.cursor()
...
class Manager:
    ...
    def __init__(self):
        ...
        # Monkey-patch _on_message to route by topic
        original_on_message = self.mqtt._on_message

        def routing_on_message(client, userdata, msg):
            payload = json.loads(msg.payload.decode())
            with self._lock:
                self._inbox[msg.topic] = payload
            print(f"[MANAGER] ← {msg.topic}: {payload}")

        self.mqtt.client.on_message = routing_on_message

        self.state                   = State.IDLE
        self.uid       : str | None  = None
        self.rfids     : list        = []
        self.vehicle   : dict | None = None   # DB row
        self.positions : list        = []
        self.cycle     : int         = 0      # 1-indexed
    ...
    def _pop(self, topic: str) -> dict | None:
        with self._lock:
            return self._inbox.pop(topic, None)

    def _cam(self, **kw):
        self.mqtt.publish("manager/camera", kw)

    def _barrier(self, **kw):
        self.mqtt.publish("manager/plc_barrier", kw)

    def _sampler(self, **kw):
        self.mqtt.publish("manager/plc_sampler", kw)
    ...
    def _resolve_rfid(self, rfids: list) -> tuple[str | None, dict | None]:
        """Try each RFID in the batch; return (rfid, vehicle_row) for first match."""
        for rfid in rfids:
    ...
    def _handle_idle(self):
        ...
        # Trigger cam2 capture immediately on vehicle arrival
    ...
    def _handle_db_check(self):
        ...
        if vehicle:
            ...
            if db_vehicle_already_in_front(vehicle.get("VEHICLE_NO", "")):
                print("[MANAGER] Vehicle already in front — aborting session.")
                self._reset()
                return
    ...
    def _handle_cycle_position(self):
        ...
        print(f"[MANAGER] Cycle {self.cycle}/{TOTAL_CYCLES} → position {pos}")

        self._sampler(action="set_position", x=pos["x"], y=pos["y"], cycle=self.cycle)
    ...
    def _handle_cycle_capture(self):
        ...
        if status == "position_set":
            # Start continuous cam1+cam3 capture and begin the cycle
            ...
            cycle = msg.get("cycle", self.cycle)
            print(f"[MANAGER] Discharge received for cycle {cycle}.")
            # Stop continuous capture for this cycle
    ...
    def _reset(self):
        ...
        # Flush all inboxes
        with self._lock: self._inbox.clear()
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                import traceback
                print(f"[MANAGER] Unhandled exception in state {self.state.name}: {e}")
                traceback.print_exc()
                self._goto(State.ERROR)
